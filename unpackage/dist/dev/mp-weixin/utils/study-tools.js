...
function runStudyTool(toolId, form) {
  if (toolId === "meihua")
    return runMeihua(form);
  if (toolId === "bazi")
    return runBazi(form);
  if (toolId === "ganzhi")
    return runGanzhi(form);
  if (toolId === "zodiac")
    return runZodiac(form);
  if (toolId === "name")
    return runName(form);
...
function runMeihua(form) {
  ...
  const date = form.date || ymd(/* @__PURE__ */ new Date());
...
function runBazi(form) {
  ...
  const dayGanzhi = cyclePair(Math.floor(d.getTime() / 864e5) + 40);
...
function runGanzhi(form) {
  ...
  const day = cyclePair(Math.floor(d.getTime() / 864e5) + 40);
...
function runZodiac(form) {
  ...
  const year = Number(form.year || (/* @__PURE__ */ new Date()).getFullYear());
  const idx = (year - 4) % 12;
...
exports.runStudyTool = runStudyTool;
//# sourceMappingURL=../../.sourcemap/mp-weixin/utils/study-tools.js.map
